...
import { handleUpdateSociety } from '@/controllers/society.controller'
...
export function EditSocietyForm({ initialData, id }: { initialData: any, id: string }) {
  ...
  const { register, handleSubmit, control, setValue, watch, formState: { errors } } = useForm<SocietyInput>({
    ...
    defaultValues: {
      ...initialData,
      additional_contacts: initialData.additional_contacts || [],
      registered_mici: initialData.registered_mici || false,
      registered_rubf: initialData.registered_rubf || false,
      registered_dgi: initialData.registered_dgi || false,
      phone_country_prefix: initialData.phone_country_prefix || '+507',
  ...
  const onSubmit = async (data: SocietyInput) => {
    setLoading(true)
    setError(null)
    const result = await handleUpdateSociety(id, data)
    if (result.success) {
      router.push(`/dashboard/sociedades/${id}`)
    } else {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Sube nuevos documentos a esta sociedad.</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <div className="flex justify-between">
        <Link href={`/dashboard/sociedades/${id}`} className={cn(buttonVariants({ variant: 'outline' }))}>
          Cancelar
        </Link>
